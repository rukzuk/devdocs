...
function addSignatureParams(f) {
	...
	var paramsJoined = params.join(', ').replace(/(\r\n|\n|\r)/gm," ");
    f.signature = util.format( '%s(%s)', (f.signature || ''), paramsJoined );
}
...
function addAttribs(f) {
	...
	var codeblock = ".. js:function:: ";
	codeblock += util.format('%s', attribsString);
    f.attribs = codeblock;
}
        ...
        if (doclet.examples) {
            doclet.examples = doclet.examples.map(function(example) {
                ...
                if (example.match(/^\s*<caption>([\s\S]+?)<\/caption>(\s*[\n\r])([\s\S]+)$/i)) {
                    caption = RegExp.$1;
                    code    = RegExp.$3.replace(/(\r\n|\n|\r)/g,"\n\t")
                }

                return {
                    caption: caption || '',
                    code: code || example.replace(/(\r\n|\n|\r)/g,"\n\t")
                };
            });
        }
        if (doclet.see) {
